...
On the Windows target, unzip this package and run PowerShell from the extracted
...
`-AddFirewallRule` should be run from an elevated Administrator PowerShell. It
allows inbound TCP `8788` only from the Tailscale address range
`100.64.0.0/10`. Without Administrator rights, add the firewall rule manually
or run without the switch and check Windows Defender Firewall separately.

The expected Tailscale client URL is:

```text
http://WINDOWS_TAILSCALE_IP:8788/predict/predict
```

For the current target, verify from the Mac:

```bash
curl --noproxy '*' -H "Authorization: Bearer $SEPSISCARE_SERVICE_TOKEN" http://100.65.136.96:8788/predict/predict
curl --noproxy '*' -X POST http://100.65.136.96:8788/predict/predict -H "Authorization: Bearer $SEPSISCARE_SERVICE_TOKEN" -H 'Content-Type: application/json' -d '{"vitals":{"map":68},"labs":{"lactate":3.2}}'
```

## DeepSeek V4 Flash

The model server exposes a DeepSeek/OpenAI-compatible chat configuration API.
After the updated service is running on Windows, configure it from the macOS
app Settings page or with PowerShell:
